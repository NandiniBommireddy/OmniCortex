...
This repository contains the scripts to extract the MIMIC-NLE dataset from the MIMIC-CXR radiology reports. In order to download MIMIC-CXR, head [here](https://physionet.org/content/mimic-cxr-jpg/2.1.0/). More details on MIMIC-NLE are provided in our MICCAI 2022 paper:
...
# check that the python binary is referred from conda
which python
```

Then download the CXR reports zip [here](https://physionet.org/content/mimic-cxr/2.1.0/mimic-cxr-reports.zip)
