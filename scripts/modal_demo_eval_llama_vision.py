"""Evaluate Llama-3.2-11B-Vision fine-tuned with LoRA on MIMIC-NLE test set.

Usage:
    modal run scripts/modal_demo_eval_llama_vision.py --variant radlex
    modal run scripts/modal_demo_eval_llama_vision.py --variant ""
    modal run scripts/modal_demo_eval_llama_vision.py --variant primekg
"""
...
REMOTE_ANS = f"{REMOTE_ROOT}/eval/llama_vision_answers.jsonl"
...
def run_llama_vision_eval(remote_data: str, remote_train_out: str) -> str:
    import torch
...
def main(variant: str = "radlex"):
    """
    Evaluate Llama-3.2-11B-Vision fine-tuned on MIMIC-NLE.
    ...
    remote_path = run_llama_vision_eval.remote(remote_data, remote_train_out)
