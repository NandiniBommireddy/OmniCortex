"""Evaluate PaliGemma2 fine-tuned with LoRA on MIMIC-NLE test set.
...
Usage:
    modal run scripts/modal_demo_eval_hf.py --variant radlex
    modal run scripts/modal_demo_eval_hf.py --variant ""
    modal run scripts/modal_demo_eval_hf.py --variant primekg
"""
...
REMOTE_ANS = f"{REMOTE_ROOT}/eval/paligemma_answers.jsonl"
...
def run_paligemma_eval(remote_data: str, remote_train_out: str) -> str:
    import torch
    ...
    processor = AutoProcessor.from_pretrained(remote_train_out)
    ...
    model.eval()
    torch._dynamo.config.disable = True  # skip torch.compile on first generate
...
def main(variant: str = "radlex"):
    """
    Evaluate PaliGemma2 fine-tuned on MIMIC-NLE.
    ...
    remote_path = run_paligemma_eval.remote(remote_data, remote_train_out)
