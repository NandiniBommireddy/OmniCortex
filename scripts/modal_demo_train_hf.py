"""Train PaliGemma2 with LoRA on MIMIC-NLE.

Uses PaliGemmaForConditionalGeneration with frozen vision tower.
Requires HuggingFace token (gated model): modal secret create huggingface HF_TOKEN=hf_xxx

Usage:
    modal run scripts/modal_demo_train_hf.py --variant radlex
    modal run scripts/modal_demo_train_hf.py --variant ""
    modal run scripts/modal_demo_train_hf.py --variant primekg
"""
...
def run_paligemma_train(remote_data: str, remote_out: str) -> list[str]:
    import torch
    ...
    class NLEDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            # PaliGemma format: prefix=question, suffix=answer
...
def main(variant: str = "radlex"):
    """
    Train PaliGemma2 on MIMIC-NLE with KG augmentation.
    ...
    remote_paths = run_paligemma_train.remote(remote_data, remote_out)
