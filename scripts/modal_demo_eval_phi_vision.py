"""Evaluate Phi-3.5-vision-instruct fine-tuned with LoRA on MIMIC-NLE test set.

Usage:
    modal run scripts/modal_demo_eval_phi_vision.py --variant radlex
    modal run scripts/modal_demo_eval_phi_vision.py --variant ""
    modal run scripts/modal_demo_eval_phi_vision.py --variant primekg
"""
...
def run_phi_vision_eval(remote_data: str, remote_train_out: str, remote_ans: str) -> str:
    import torch
    ...
    processor = AutoProcessor.from_pretrained(
        remote_train_out, trust_remote_code=True, num_crops=4
    )
...
def main(variant: str = "radlex"):
    """
    Evaluate Phi-3.5-vision-instruct fine-tuned on MIMIC-NLE.
    ...
    remote_ans = f"{REMOTE_ROOT}/eval/phi_vision_answers{suffix}.jsonl"
    ...
    remote_path = run_phi_vision_eval.remote(remote_data, remote_train_out, remote_ans)
